...
from .forms import TopicForm, EntryForm, QuestionForm
...
subsections = [["School Name", "Attendance Period", "GPA", "Notable Events and Achievements"], ["Test/Competition Name", "Score", "Skills Worked On"], ["Course Name", "Mark", "Things Learned", "Enjoyment"], ["Extracurricular Name", "Activities", "Skills Honed", "Notable Achievements"]]
...
loader = TextLoader(r"C:\Anaconda3\envs\WebDev\kprjs\media\measurements.txt")
...
education_questions = {
  # key -> [question, prompt, answer, result]
  'nextQuestion' : ['General', 0, 0, 0],
  'General' : ['Please tell me something about your education such as school, graducation year, and GPA.', 'Please extract school, year, and GPA from this text', "", ""],
  'School' : ['Please tell me your school name.', 'Please extract school from this text', "", ""],
  'Year' : ['Please tell me your graducation year', 'Please extract graduation year from this text', "", ""],
  'GPA' : ['Please tell me your GPA', 'Please extract GPA from this text', "", ""],
  }

'''
Extracurriculars_questions = [
  0, # current question index
  ['General', 'Please tell me some information about your extracurricular activities.', False],
  ['More', 'Do you have more extracurricular activities?', False],
  ['', 'Please tell me about your achivement in ', False],
  ]
'''
...
def profile_agent_get_fieldinfo(input_info, output_info, delimiter = ':'):
  ...
  if index >= 0:
    ...
    if index >= 0:
      ...
      index2 = text.find('\n', index)
      if index2 >= 0:
        information = text[index : index2].strip()

  return information
  '''

def profile_agent_next_question(questions_type):

  if questions_type == 'education':
    for question_key in education_questions:
      if education_questions[question_key][2] == "" or education_questions[question_key][3] == "":
        return question_key, education_questions[question_key][0]
    return "", ""

def profile_agent_save_answer(questions_type, question_key, answer):

  if questions_type == 'education':
    education_questions[question_key][2] = answer
...
def topic(request, topic_id):
    ...
    context = {'topic': topic, 'form': EntryForm(), 'entries': entries, 'profile_chat_history' : profile_chat_history, 'question': prefix + get_question(request, topic_id),
               'ai_suggested_texts' : ai_suggested_texts, 'incomplete_entries' : incomplete_entries,
...
def new_topic(request):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            new_topic.save()
            return redirect('resume_logs:topics')

    # Display a blank or invalid form.
    context = {'form': form}
    return render(request, 'resume_logs/new_topic.html', context)
...
def new_entry(request, topic_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
                #Entry.objects.create(topic=topic, sender='Resume Assistant', text=question)
            topics = Topic.objects.filter(owner=request.user).order_by('date_added')
            local_topic_id = topic_id - topics[0].id
...
def edit_entry(request, entry_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('resume_logs:topic', topic_id=topic.id)

    context = {'entry': entry, 'topic': topic, 'form': form}
    return render(request, 'resume_logs/edit_entry.html', context)
  
  
@login_required
def topics_editprofile(request):
  topics = Topic.objects.filter(owner=request.user).order_by('id')
  context = {'topics' : topics}
  return render(request, 'resume_logs/topics_editprofile.html', context)


@login_required
def edit_entry_editprofile(request, entry_id):
    """Edit an existing entry."""
    entry = Entry.objects.get(id=entry_id)
    topic = entry.topic
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('resume_logs:topic_editprofile', topic_id=topic.id)

    context = {'entry': entry, 'topic': topic, 'form': form}
    return render(request, 'resume_logs/edit_entry_editprofile.html', context)
...
def generate_txt(request):
    ...
    with open(file_path, 'rb') as file:
        response = HttpResponse(file.read(), content_type='application/octet-stream')
        response['Content-Disposition'] = f'attachment; filename=' + RESULT_FILE_TXT
        return response
...
def generate_pdf(request):
    ...
    #print(f'==========={pdf.bottom_margin}=={pdf.left_margin}')
    ...
    with open(file_path, 'rb') as file:
        response = HttpResponse(file.read(), content_type='application/octet-stream')
        response['Content-Disposition'] = f'attachment; filename=' + RESULT_FILE_PDF
        return response
...
def generate_docx(request):
    ...
    with open(file_path, 'rb') as file:
        response = HttpResponse(file.read(), content_type='application/octet-stream')
        response['Content-Disposition'] = f'attachment; filename=' + RESULT_FILE_DOC
        return response
